...
Étendre le système de tasks existant avec le modèle "Fast-Food Workflow" : tasks parent avec sous-tasks qui progressent entre secteurs.
...
### Backend
- `server/api/studio.py` — Routes API tasks existantes (`/api/studio/tasks`, `/api/studio/tasks/sync`)

### Frontend
- `src/stores/studioStore.ts` — Store Zustand complet (300 lignes) avec :
  - `Task` interface (id, title, description, sector_id, status, priority, agent_id, context)
  - `Sector` interface (id, name, icon, color, order)
  - Actions : addTask, updateTask, removeTask, startTask, completeTask, reopenTask
  - Computed : getTasksBySector, getSuggestedTask
  - Sync filesystem : loadFromProject, syncToProject

### Ce qui MANQUE (à ajouter)
- Dépendances entre tasks (`requires: []`)
- Modèle parent/subtasks
- Historique append-only
- Status `locked` / `ready` calculé automatiquement
- Secteurs personnalisables (4 par défaut + custom user/LLM)

## Fichiers à Modifier

- `web-ui/server/api/studio.py` — Étendre les routes existantes
- `web-ui/src/stores/studioStore.ts` — Ajouter les features manquantes
- `web-ui/src/types/tasks.ts` — Créer (types séparés du store)
