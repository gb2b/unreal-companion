...
Créer un template CLAUDE.md généré automatiquement pour chaque projet, avec le contexte projet, les workflows disponibles, et les instructions pour les agents.
...
- `web-ui/server/services/claudemd_service.py` - Génération dynamique
...
│   ├── tasks/              # Production board tasks
│   └── context.md          # AI context (this feeds me)
...
|-------|------|-------------|
| 🎲 Game Designer | Mechanics, gameplay | Design discussions |
| 🏛️ Architect | Systems, architecture | Technical planning |
| 🎨 3D Artist | Visuals, materials | Art direction |
| 🗺️ Level Designer | Levels, flow | World building |
| 🤖 Unreal Agent | MCP execution | Building in UE |
...
class ClaudeMDService:
    ...
    def generate(self) -> str:
        """Génère le contenu du CLAUDE.md."""
    ...
    def get_current_focus(self) -> str:
        """Détermine le focus actuel depuis les tasks."""
    ...
    def format_active_tasks(self) -> str:
        ...
        for t in tasks[:5]:  # Top 5
            status_icon = {'ready': '▶', 'in_progress': '🔄', 'locked': '🔒'}.get(t['status'], '○')
            lines.append(f"- {status_icon} {t['title']} ({t['sector']})")

        return "\n".join(lines)

    def write_to_project(self):
        """Écrit le CLAUDE.md à la racine du projet."""
...
```python
# Dans project_init.py ou au démarrage du serveur
def on_project_change(project_path: str, change_type: str):
    """Régénère CLAUDE.md quand le projet change."""
...
# Force la régénération du CLAUDE.md
```

## Critères d'Acceptation

- [ ] Template CLAUDE.md complet
- [ ] Génération automatique à l'init du projet
- [ ] Mise à jour quand context.md change
- [ ] Mise à jour quand artifacts ajoutés
- [ ] Mise à jour quand tasks changent
...
def test_generate_claude_md():
    # Vérifie la génération complète

def test_artifacts_list_formatting():
    # Vérifie le format de la liste

def test_active_tasks_formatting():
    # Vérifie le format des tasks

def test_regeneration_on_change():
    # Vérifie la régénération automatique
```

## Notes

- Le CLAUDE.md doit être lisible par tout assistant IA
- Garder le format simple et scannable
- Éviter de répéter tout le GDD (juste pointer vers)
- Prévoir un mode verbose pour plus de contexte si besoin
