# P2.4 Workflows YAML (Définitions)

## Objectif

Créer les définitions YAML des workflows principaux : quick-start, game-brief, gdd, brainstorm. Ces fichiers définissent la structure, les étapes, et les questions.

## Prérequis

- [x] P1.1 - Structure globale avec workflows/defaults/

## Fichiers à Créer

```
~/.unreal-companion/workflows/defaults/
├── quick-start.yaml       # 2 min - pitch rapide
├── game-brief.yaml        # 15-30 min - vision complète
├── gdd.yaml               # 1h+ - design document
├── brainstorm.yaml        # 10 min - idées libres
└── sprint-planning.yaml   # 15 min - planifier les tasks
```

## Spécifications

### Format Workflow YAML

```yaml
id: game-brief
version: "1.0"

# Métadonnées
metadata:
  name:
    en: "Game Brief"
    fr: "Brief de Jeu"
  description:
    en: "Define your game vision, genre, and core mechanics"
    fr: "Définir la vision, le genre et les mécaniques de base"
  icon: "🎯"
  color: "#3B82F6"
  estimated_duration: "15-30 min"
  category: "conception"  # conception | production | brainstorm

# Agent par défaut
agent: game-designer

# Output configuration
output:
  type: markdown
  path: "artifacts/briefs/game-brief.md"
  update_context: true  # Met à jour context.md

# Étapes du workflow
steps:
  - id: vision
    title:
      en: "Core Vision"
      fr: "Vision Principale"

    # Message de l'agent (dynamique, utilisé comme guide)
    agent_message:
      en: "Let's capture the essence of your game! What's the one-sentence pitch that makes people go 'I want to play that!'?"
      fr: "Définissons l'essence de ton jeu ! Quel est le pitch en une phrase qui donne envie de jouer ?"

    # Questions de l'étape
    questions:
      - id: elevator_pitch
        type: textarea
        prompt:
          en: "Describe your game in one compelling sentence"
          fr: "Décris ton jeu en une phrase percutante"
        placeholder:
          en: "In [Game Name], you play as... who must... by..."
          fr: "Dans [Nom du Jeu], tu incarnes... qui doit... en..."
        required: true
        validation:
          min_length: 20
          max_length: 500

      - id: core_fantasy
        type: textarea
        prompt:
          en: "What fantasy does the player experience?"
          fr: "Quelle expérience/fantasme le joueur vit-il ?"
        description:
          en: "The core feeling or power fantasy that drives engagement"
          fr: "Le sentiment ou la fantaisie de pouvoir qui engage le joueur"
        required: true

    # Milestone optionnel après cette étape
    milestone:
      name:
        en: "Vision Defined"
        fr: "Vision Définie"
      celebration:
        en: "Your core vision is set! This is the foundation of everything."
        fr: "Ta vision est posée ! C'est la fondation de tout le reste."

  - id: genre
    title:
      en: "Genre & References"
      fr: "Genre & Références"

    agent_message:
      en: "Great vision! Now let's position it. What genres inspire your game?"
      fr: "Excellente vision ! Maintenant, positionnons-la. Quels genres t'inspirent ?"

    questions:
      - id: primary_genre
        type: single
        prompt:
          en: "What's the primary genre?"
          fr: "Quel est le genre principal ?"
        options:
          - id: action
            label:
              en: "Action"
              fr: "Action"
          - id: adventure
            label:
              en: "Adventure"
              fr: "Aventure"
          - id: rpg
            label:
              en: "RPG"
              fr: "RPG"
          - id: puzzle
            label:
              en: "Puzzle"
              fr: "Puzzle"
          - id: simulation
            label:
              en: "Simulation"
              fr: "Simulation"
          - id: strategy
            label:
              en: "Strategy"
              fr: "Stratégie"
          - id: other
            label:
              en: "Other"
              fr: "Autre"
            allow_custom: true
        required: true

      - id: references
        type: multiple
        prompt:
          en: "Which games inspire you? (select all that apply)"
          fr: "Quels jeux t'inspirent ? (sélectionne tous ceux qui s'appliquent)"
        description:
          en: "The LLM will suggest based on your genre"
          fr: "Le LLM suggérera en fonction de ton genre"
        suggestions_from_context: true  # LLM génère des suggestions
        min_selections: 1
        max_selections: 5
        allow_custom: true

  - id: mechanics
    title:
      en: "Core Mechanics"
      fr: "Mécaniques Principales"

    # Condition d'affichage optionnelle
    show_if:
      question: primary_genre
      not_equals: puzzle  # Skip si puzzle (questions différentes)

    agent_message:
      en: "Now the fun part - what does the player actually DO?"
      fr: "Maintenant le plus fun - que fait concrètement le joueur ?"

    questions:
      - id: core_loop
        type: textarea
        prompt:
          en: "Describe the main gameplay loop"
          fr: "Décris la boucle de gameplay principale"
        description:
          en: "What actions repeat? What's the reward cycle?"
          fr: "Quelles actions se répètent ? Quel est le cycle de récompense ?"
        required: true

      - id: unique_mechanic
        type: textarea
        prompt:
          en: "What makes your game unique mechanically?"
          fr: "Qu'est-ce qui rend ton jeu unique mécaniquement ?"
        required: false

  # Étape finale
  - id: summary
    title:
      en: "Summary"
      fr: "Résumé"

    agent_message:
      en: "Awesome! Let me summarize what we've defined..."
      fr: "Super ! Laisse-moi résumer ce qu'on a défini..."

    # Pas de questions, juste génération du document
    generate_summary: true

# Célébration finale
completion:
  celebration:
    en: "🎉 Game Brief complete! You have a solid foundation for {{project_name}}."
    fr: "🎉 Brief terminé ! Tu as une base solide pour {{project_name}}."
  next_suggestions:
    - workflow: gdd
      label:
        en: "Create full GDD"
        fr: "Créer le GDD complet"
    - workflow: brainstorm
      label:
        en: "Brainstorm mechanics"
        fr: "Brainstormer les mécaniques"
```

### quick-start.yaml (simplifié)

```yaml
id: quick-start
version: "1.0"

metadata:
  name:
    en: "Quick Start"
    fr: "Démarrage Rapide"
  description:
    en: "Get started in 2 minutes with just the essentials"
    fr: "Démarre en 2 minutes avec l'essentiel"
  icon: "🚀"
  color: "#10B981"
  estimated_duration: "2 min"
  category: "conception"

agent: game-designer

output:
  type: markdown
  path: "artifacts/briefs/quick-brief.md"
  update_context: true

steps:
  - id: basics
    title:
      en: "The Basics"
      fr: "L'Essentiel"

    agent_message:
      en: "Let's get the essentials down quickly! What's your game about?"
      fr: "Allons à l'essentiel ! C'est quoi ton jeu ?"

    questions:
      - id: name
        type: text
        prompt:
          en: "What's the project name?"
          fr: "Comment s'appelle le projet ?"
        required: true

      - id: pitch
        type: textarea
        prompt:
          en: "One-sentence pitch"
          fr: "Pitch en une phrase"
        required: true

      - id: genre
        type: single
        prompt:
          en: "Genre?"
          fr: "Genre ?"
        options:
          - { id: action, label: { en: "Action", fr: "Action" } }
          - { id: rpg, label: { en: "RPG", fr: "RPG" } }
          - { id: puzzle, label: { en: "Puzzle", fr: "Puzzle" } }
          - { id: other, label: { en: "Other", fr: "Autre" }, allow_custom: true }
        required: true

completion:
  celebration:
    en: "🚀 You're set! Your project is ready to explore."
    fr: "🚀 C'est parti ! Ton projet est prêt."
  next_suggestions:
    - workflow: game-brief
      label:
        en: "Deep dive with full Game Brief"
        fr: "Approfondir avec le Brief complet"
```

## Critères d'Acceptation

- [ ] quick-start.yaml créé et valide
- [ ] game-brief.yaml créé avec toutes les étapes
- [ ] gdd.yaml créé (peut être partiel, structure définie)
- [ ] brainstorm.yaml créé
- [ ] sprint-planning.yaml créé
- [ ] Tous les fichiers ont les traductions en/fr
- [ ] Format YAML validé (pas d'erreurs de syntaxe)
- [ ] Champs obligatoires présents (id, steps, output)
- [ ] Tests de parsing passent

## Tests à Écrire

### Unit Tests

```python
def test_parse_workflow_yaml():
    # Vérifie que tous les workflows se parsent sans erreur

def test_workflow_has_required_fields():
    # Vérifie id, steps, output pour chaque workflow

def test_workflow_translations_complete():
    # Vérifie que en/fr sont présents partout

def test_question_types_valid():
    # Vérifie que les types sont supportés
```

## Types de Questions Supportés

### Types de Base

| Type | Description | Exemple |
| ---- | ----------- | ------- |
| `text` | Champ texte une ligne | Nom du projet |
| `textarea` | Champ texte multi-lignes | Description, pitch |
| `single` | Choix unique (radio) | Genre principal |
| `multiple` | Choix multiples (checkbox) | Références, tags |

### Types Interactifs (voir P2.5)

| Type | Description | Exemple |
| ---- | ----------- | ------- |
| `spectrum` | Jauge entre deux extrêmes | Linéaire ↔ Non-linéaire |
| `emotional_scale` | Échelle d'émojis | 😰 😐 🤔 😮 🤩 |
| `visual_ab` | Choix A/B avec visuels | "Dark Souls" vs "XCOM" |
| `appreciation` | Jauge de feedback | 👎 😐 👍 ❤️ 🔥 |

### Exemple de Question Interactive

```yaml
questions:
  - id: progression_style
    type: spectrum
    prompt:
      en: "How linear should the progression be?"
      fr: "À quel point la progression doit être linéaire ?"
    extremes:
      left:
        label: { en: "Linear", fr: "Linéaire" }
        description: { en: "Guided path", fr: "Chemin guidé" }
      right:
        label: { en: "Open", fr: "Ouvert" }
        description: { en: "Player choice", fr: "Choix du joueur" }
    allow_comment: true  # Permet un commentaire optionnel

  - id: target_emotion
    type: emotional_scale
    prompt:
      en: "What emotion should the player feel?"
      fr: "Quelle émotion le joueur doit ressentir ?"
    scale:
      - { emoji: "😰", label: { en: "Anxiety", fr: "Anxiété" } }
      - { emoji: "😐", label: { en: "Calm", fr: "Calme" } }
      - { emoji: "🤔", label: { en: "Curiosity", fr: "Curiosité" } }
      - { emoji: "😮", label: { en: "Wonder", fr: "Émerveillement" } }
      - { emoji: "🤩", label: { en: "Euphoria", fr: "Euphorie" } }
```

### Quick Interactions (optionnelles dans workflow)

```yaml
# Peut être ajouté dans une step pour débloquer les idées
quick_interaction:
  type: speed_round  # speed_round | this_or_that | word_association | wild_card
  config:
    word_count: 5      # Pour speed_round
    timer: 15          # Secondes (optionnel, 0 = pas de timer)
    questions_count: 10  # Pour this_or_that
```

## Architecture Step-File (BMGD Pattern)

> Pour les workflows complexes (game-brief, gdd), utiliser l'architecture step-file de BMGD.
> Source : [bmad-module-game-dev-studio](https://github.com/bmad-code-org/bmad-module-game-dev-studio)

### Structure dossier (workflows complexes)

```
~/.unreal-companion/workflows/defaults/game-brief/
├── workflow.yaml              # Config principale (métadonnées, output)
├── instructions.md            # Guidance LLM générale
├── checklist.md               # Critères de validation finale
├── steps/
│   ├── step-01-init.md        # Détection état + init frontmatter
│   ├── step-01b-continue.md   # Reprise session existante
│   ├── step-02-vision.md      # Étape vision
│   ├── step-03-genre.md       # Étape genre
│   ├── step-04-mechanics.md   # Étape mécaniques
│   └── step-05-complete.md    # Finalisation + validation
└── templates/
    └── game-brief-template.md  # Template avec {{variables}}
```

### Règles LLM obligatoires (en haut de chaque step)

```markdown
## MANDATORY EXECUTION RULES

- Ne pas dévier, skip, ou optimiser la séquence
- JAMAIS générer de contenu sans input utilisateur
- TOUJOURS lire le fichier step ENTIER avant exécution
- JAMAIS charger plusieurs step files en même temps
- Compléter la lecture avant d'agir
```

### Menu-driven progression

```markdown
## OPTIONS UTILISATEUR

[A] Accepter l'output actuel
[P] Donner du feedback / demander des modifications
[C] Continuer vers l'étape suivante
[AE] Advanced Elicitation (questions approfondies)

⚠️ SEUL [C] Continue sauvegarde l'état et avance le workflow
```

### Frontmatter state tracking

```yaml
---
workflowId: game-brief
workflowName: Game Brief
workflowVersion: 1.0.0
projectName: "{{project_name}}"
stepsCompleted: [1, 2]         # Array des étapes terminées
currentStep: 3
lastUpdated: 2024-01-22T14:32:00Z
workflowStatus: in-progress     # in-progress | paused | complete
inputDocuments:
  - type: brainstorming
    name: Ideas.md
lastEditorAgent: game-designer
---
```

### Exemple step file (step-02-vision.md)

```markdown
## MANDATORY EXECUTION RULES
[règles ci-dessus]

## ROLE: Game Designer (Zelda)
Agis en facilitateur passionné. Style enthousiaste mais structuré.

## PRIMARY GOAL
Capturer la vision core du jeu en une phrase percutante.

## DISCOVERY PATHS
- PATH A (Brief existant) : Proposer de confirmer/ajuster
- PATH B (Brainstorm existant) : Extraire les idées principales
- PATH C (Nouveau) : Questions guidées

## EXECUTION SEQUENCE
1. Saluer et rappeler le contexte
2. Poser la question du pitch
3. Attendre la réponse utilisateur
4. Réagir positivement et reformuler
5. Poser la question du core fantasy
6. Synthétiser
7. Afficher le menu [A] [P] [C] [AE]

## SUCCESS CRITERIA
✓ Pitch en une phrase (20-200 caractères)
✓ Core fantasy identifié
✓ Utilisateur satisfait (a choisi [A] ou [C])

## FAILURE CRITERIA
✗ Pitch trop vague ou générique
✗ Pas de core fantasy défini
✗ Skip sans input utilisateur
```

## Notes

- Les workflows sont en YAML pour être lisibles/éditables par les utilisateurs
- Le LLM utilise ces définitions mais génère les messages dynamiquement
- `suggestions_from_context: true` indique que le LLM doit suggérer des options
- Les conditions `show_if` permettent des workflows adaptatifs
- Les types interactifs rendent l'expérience plus ludique (voir P2.5)
- Pour les workflows simples (quick-start), un seul YAML suffit
- Pour les workflows complexes (game-brief, gdd), utiliser l'architecture step-file
